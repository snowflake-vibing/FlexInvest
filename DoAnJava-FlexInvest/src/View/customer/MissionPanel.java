...
public class MissionPanel extends JPanel {
    ...
    private JPanel buildCheckInTab() {
        ...
        p.setBackground(UIUtils.APP_BG);
        p.setBorder(new EmptyBorder(20, 20, 20, 20));

        // Title
        JLabel title = UIUtils.createHeading2("Chuỗi điểm danh liên tiếp 7 ngày");
        p.add(title, BorderLayout.NORTH);

        // 7 ô điểm danh
        JPanel grid = new JPanel(new GridLayout(1, 7, 10, 0));
        grid.setBackground(UIUtils.APP_BG);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            try {
                faFont = java.awt.Font.createFont(java.awt.Font.TRUETYPE_FONT,
                    new java.io.File("src/Resources/fa-solid-900.ttf")).deriveFont(30f);
            } catch (Exception ex) {
                faFont = new Font("Segoe UI", Font.PLAIN, 28);
            }

            JLabel icon = new JLabel("\uf111", SwingConstants.CENTER); // FA circle
            icon.setFont(faFont);
    ...
    private void updateCheckInUI(int streak) {
        for (int i = 0; i < 7; i++) {
            ...
            // \uf00c = FA check, \uf005 = FA star solid, \uf111 = FA circle
    ...
    private boolean alreadyCheckedInToday() {
        return missionDAO.hasCheckedInToday(userId);
    }
    ...
    private JPanel buildMissionListPanel(List<UserMission> missions) {
        ...
        sp.setBorder(null);
        sp.getViewport().setBackground(UIUtils.APP_BG);
    ...
    private void onCheckIn() {
        ...
        new SwingWorker<Boolean, Void>() {
            @Override protected Boolean doInBackground() {
                return missionDAO.checkIn(userId); // Token được cộng trong MissionDAO.doCheckIn()
            }
            @Override protected void done() {
                try {
                    boolean ok = get();
