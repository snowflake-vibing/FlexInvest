...
public class MissionDAO {
    ...
    private boolean doCheckIn(int userId, int missionId, int targetValue) {
        ...
        if (um == null) {
            ...
            try (Connection conn = ConnectionOracle.getOracleConnection();
                ...
                if (inserted) {
                    // Cộng token phần thưởng từ MISSIONS.reward_token
                    Mission m = getMissionById(missionId);
                    if (m != null && m.getRewardToken() != null) {
                        boolean tokenOk = tokenDAO.addToken(userId, m.getRewardToken());
                        if (!tokenOk) System.err.println("[MissionDAO.doCheckIn] Cộng token thất bại cho user " + userId);
                    }
                }
                return inserted;
            } catch (Exception e) {
    ...
    public int getCheckInStreak(int userId) {
        ...
        // Kiểm tra có bị reset chưa (bỏ lỡ 1 ngày)
    ...
    public boolean hasCheckedInToday(int userId) {
        Mission checkInMission = getCheckInMission();
        if (checkInMission == null) return false;
        
        UserMission um = getUserMission(userId, checkInMission.getMissionId());
        if (um != null && um.getClaimedAt() != null) {
            LocalDate lastCheckIn = um.getClaimedAt().toLocalDateTime().toLocalDate();
            return lastCheckIn.isEqual(LocalDate.now());
        }
        return false;
    }
