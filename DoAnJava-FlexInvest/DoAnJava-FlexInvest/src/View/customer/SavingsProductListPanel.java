...
public class SavingsProductListPanel extends JPanel {
    ...
    private void build() {
        ...
        JLabel title = UIUtils.createHeading1("Gói Đầu Tư");
    ...
    private void onBuyClicked(int row) {
        ...
        // Load wallet trên background thread để không block EDT
        btnRefresh.setEnabled(false);
        setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        new SwingWorker<Wallet, Void>() {
            @Override protected Wallet doInBackground() { return walletDAO.getByUserId(userId); }
            @Override protected void done() {
                setCursor(Cursor.getDefaultCursor());
                btnRefresh.setEnabled(true);
                try {
                    showBuyDialog(p, get(), row);
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(SavingsProductListPanel.this,
                        "Lỗi tải dữ liệu ví. Vui lòng thử lại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            }
        }.execute();
    }

    private void showBuyDialog(SavingsProduct p, Wallet wallet, int row) {
        // ── Custom Dialog nhập số tiền ─────────────────────────────────────────────
        ...
        String balStr = wallet != null ? VND.format(wallet.getAvailableBalance()) : "0";

        JDialog dlg = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Đăng ký mua gói", true);
        dlg.setSize(420, 360);
        ...
        JLabel lblTitle = UIUtils.createHeading2("Gói " + p.getProductName());
        
        JLabel lblSub = new JLabel("Lãi suất: " + p.getInterestRate().multiply(BigDecimal.valueOf(100)).toPlainString() + "%/năm");
        lblSub.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        lblSub.setForeground(TEXT_MUTED);
        
        JLabel lblLimit = new JLabel("Hạn mức: " + minStr + " - " + maxStr + " đ");
        lblLimit.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        lblLimit.setForeground(BLUE);
        
        JLabel lblBal = new JLabel("Số dư khả dụng: " + balStr + " đ");
        lblBal.setFont(new Font("Segoe UI", Font.BOLD, 13));
        lblBal.setForeground(GREEN);
        
        JLabel lblInput = new JLabel("Nhập số tiền muốn đầu tư:");
        lblInput.setFont(new Font("Segoe UI", Font.BOLD, 13));
        lblInput.setForeground(TEXT_DARK);
        ...
        txtAmount.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(BORDER_C, 1), new EmptyBorder(5, 10, 5, 10)));
        
        JLabel lblErr = new JLabel(" ");
        lblErr.setFont(new Font("Segoe UI", Font.ITALIC, 12));
        lblErr.setForeground(RED);
        
        pnl.add(lblTitle); pnl.add(Box.createVerticalStrut(4));
        pnl.add(lblSub); pnl.add(Box.createVerticalStrut(12));
        pnl.add(lblLimit); pnl.add(Box.createVerticalStrut(4));
        pnl.add(lblBal); pnl.add(Box.createVerticalStrut(24));
        pnl.add(lblInput); pnl.add(Box.createVerticalStrut(8));
        pnl.add(txtAmount); pnl.add(Box.createVerticalStrut(4));
        pnl.add(lblErr);
        
        JPanel bot = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 12));
        bot.setBackground(UIUtils.APP_BG);
        bot.setBorder(new MatteBorder(1, 0, 0, 0, BORDER_C));
        JButton btnCancel = new JButton("Hủy");
        btnCancel.setFont(new Font("Segoe UI", Font.BOLD, 13));
        btnCancel.setBackground(Color.WHITE);
        btnCancel.setFocusPainted(false);
        JButton btnOk = new JButton("Tiếp tục");
        btnOk.setFont(new Font("Segoe UI", Font.BOLD, 13));
        btnOk.setBackground(BLUE); btnOk.setForeground(Color.WHITE);
        btnOk.setFocusPainted(false);
        bot.add(btnCancel); bot.add(btnOk);
        
        dlg.add(pnl, BorderLayout.CENTER);
        dlg.add(bot, BorderLayout.SOUTH);
        
        final BigDecimal[] resultAmount = {null};
        
        btnCancel.addActionListener(e -> dlg.dispose());
        btnOk.addActionListener(e -> {
            try {
                String raw = txtAmount.getText().trim().replace(",", "").replace(".", "");
                if (raw.isEmpty()) { lblErr.setText("Vui lòng nhập số tiền"); return; }
                BigDecimal amt = new BigDecimal(raw);
                if (amt.compareTo(p.getMinInvestmentAmount()) < 0) {
                    lblErr.setText("Tối thiểu " + minStr + " đ"); return;
                }
                if (p.getMaxInvestmentAmount() != null && amt.compareTo(p.getMaxInvestmentAmount()) > 0) {
                    lblErr.setText("Tối đa " + maxStr + " đ"); return;
                }
                if (wallet == null || wallet.getAvailableBalance().compareTo(amt) < 0) {
                    lblErr.setText("Số dư không đủ! (Hiện có: " + balStr + " đ)"); return;
                }
                resultAmount[0] = amt;
                dlg.dispose();
            } catch (Exception ex) {
                lblErr.setText("Số tiền không hợp lệ");
            }
        });
        
        dlg.setVisible(true);
        if (resultAmount[0] == null) return;
        BigDecimal amount = resultAmount[0];
