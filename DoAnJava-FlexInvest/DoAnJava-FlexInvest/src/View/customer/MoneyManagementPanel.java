...
public class MoneyManagementPanel extends JPanel {
    ...
    private void onDeleteBank() {
        int idx = lstBanks.getSelectedIndex();
        if (idx < 0 || bankListFull == null || idx >= bankListFull.size()) {
            warn("Vui lòng chọn tài khoản cần xóa!"); return;
        }
        BankAccount selected = bankListFull.get(idx);
        int confirm = JOptionPane.showConfirmDialog(this,
            "Xóa tài khoản " + selected.getBankName() + " — " + selected.getAccountNumber() + "?",
            "Xác nhận xóa", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.YES_OPTION) return;

        boolean ok = bankDAO.softDelete(selected.getBankAccountId(), userId);
        if (ok) {
            loadBankList();
            loadBankCombo();
            JOptionPane.showMessageDialog(this, "Đã xóa tài khoản ngân hàng!");
        } else {
