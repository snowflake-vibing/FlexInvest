...
public class StaffDashboardPanel extends JPanel {
    ...
    private void build() {
        ...
        quickRow.add(makeLinkBtn("Duyệt Nạp",  new Color(0, 120, 212), e -> { if(onNavigate!=null) onNavigate.accept("STAFF_DEPOSIT"); }));
        quickRow.add(makeLinkBtn("Duyệt Rút",   new Color(245, 100, 0), e -> { if(onNavigate!=null) onNavigate.accept("STAFF_WITHDRAW"); }));
        quickRow.add(makeLinkBtn("Duyệt KYC",   new Color(16, 185, 129), e -> { if(onNavigate!=null) onNavigate.accept("STAFF_KYC"); }));
        inner.add(quickRow);
    ...
    private JButton makeLinkBtn(String text, Color fg, java.awt.event.ActionListener l) {
        JButton b = new JButton(text);
